...
export default function App() {
  ...
  useEffect(() => {
    if (navHidden || isDesktop) {
      ...
      document.documentElement.style.setProperty("--nav-h", "95px");
    }
  }, [navHidden, isDesktop]);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: "'Inter','Helvetica Neue',sans-serif", display: "flex" }}>
      ...
      {/* モバイルボトムナビ */}
      {!isDesktop && (<>
        <button onClick={() => setNavHidden(!navHidden)} style={{ position: "fixed", bottom: navHidden ? 8 : "calc(var(--nav-h, 95px) + env(safe-area-inset-bottom, 0px) + 3px)", right: 10, zIndex: 200, background: "rgba(26,25,41,0.95)", border: `1px solid ${C.cardBorder}`, borderRadius: 10, width: 32, height: 32, cursor: "pointer", color: C.sub, fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center" }}>
          {navHidden ? "▲" : "▼"}
        </button>
        {!navHidden && (
